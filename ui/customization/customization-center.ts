...
export class CustomizationCenter {
  ...
  #build(): void {
    ...
    this.#body.append(topbar, this.#message, this.#content, footer, this.#live); this.#modal.append(this.#sidebar, this.#body, close); this.element.append(this.#modal);
  ...
  #layout(config: CustomizationConfig): void {
    ...
    this.#content.append(density, sidebar, toolbar, status);
  }
  ...
  #data(): void {
    this.#intro("Workspaces e dados", "Escopo global ou por workspace, JSON V2 validado e recuperação segura.");
    ...
    const reset = this.#group("Reset e recuperação", "Cada categoria possui reset próprio; abaixo você restaura o escopo atual ou toda a V2.", "reset backup último estado válido restaurar"); const resetActions = element("div", "moon-settings-actions"); const current = button("moon-secondary-button", "Restaurar escopo atual", "reload"); current.append(element("span", "", "Restaurar este escopo")); current.addEventListener("click", () => { if (confirm("Restaurar o escopo atual?")) { this.options.store.resetAll("current"); this.#render(); } }); const all = button("moon-danger-button", "Reset total", "trash"); all.append(element("span", "", "Reset total")); all.addEventListener("click", () => { if (confirm("Restaurar toda a personalização?")) { this.options.store.resetAll("everything"); this.#render(); } }); resetActions.append(current, all); reset.append(resetActions, element("p", "moon-recovery-note", this.options.store.loadResult.message ?? "O último estado válido é atualizado após cada mudança aceita."));
    this.#content.append(portability, reset);
  }
  ...
  #result(accepted: boolean): void { this.#undo.disabled = !this.options.store.canUndo; this.#redo.disabled = !this.options.store.canRedo; if (accepted) { this.#message.textContent = "Alteração aplicada ao vivo e salva."; this.#message.classList.remove("is-error"); } else this.#error(this.options.store.lastError ?? "Valor inválido."); }
